	...
	<body>
		...
		<div id="show-flight" class="content scaffold-show" role="main">
			...
			<ol class="property-list flight">
				...
				<g:if test="${flightInstance?.departureAirport}">
				<li class="fieldcontain">
					<span id="departureAirport-label" class="property-label"><g:message code="flight.departureAirport.label" default="Departure Airport" /></span>
					
						<span class="property-value" aria-labelledby="departureAirport-label"><g:link controller="airport" action="show" id="${flightInstance?.departureAirport?.id}">${flightInstance?.departureAirport?.encodeAsHTML()}</g:link></span>
